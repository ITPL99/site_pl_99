...
public class NewsController {
    ...
    public ResponseEntity<String> saveNews(@ModelAttribute NewsDtoRequest newsDto, @RequestParam(required = false) List<MultipartFile> file) throws BaseException {
    ...
    public ResponseEntity<?> getNewsId(@PathVariable Long id) throws BaseException {
    ...
    public ResponseEntity<?> uploadImages(@RequestParam("file") List<MultipartFile> file, @PathVariable Long newsId) throws BaseException {
            UserEntity user = (UserEntity) SecurityContextHolder.getContext().getAuthentication().getPrincipal();
            for (MultipartFile file1 : file) {
    ...
    public ResponseEntity<?> streamImages(@PathVariable Long id) throws BaseException {
    ...
    public ResponseEntity<?> uploadVideo(@RequestParam("file") MultipartFile file, @PathVariable Long newsId) throws BaseException{
            UserEntity user = (UserEntity) SecurityContextHolder.getContext().getAuthentication().getPrincipal();
    ...
    public ResponseEntity<?> streamVideo(@PathVariable Long id) throws BaseException{
        return ResponseEntity.ok()
